...
class _UpcomingEpisodeCardState extends State<UpcomingEpisodeCard> {
  ...
  void didUpdateWidget(UpcomingEpisodeCard oldWidget) {
    ...
    if (oldWidget.series != widget.series || oldWidget.anilistId != widget.anilistId) {
      _loadImageAndColor();
    }
  }
  ...
  Future<void> _loadImageAndColor() async {
    ...
    try {
      // Use the AniList ID if provided, otherwise fall back to the primary poster
      if (widget.anilistId != null) {
        _posterImageProvider = await widget.series.getPosterImageForAnilistId(widget.anilistId!);
        _posterColor = widget.series.getEffectivePosterColorForAnilistId(widget.anilistId!);
      } else {
        _posterImageProvider = await widget.series.getPosterImage();
        _posterColor = widget.series.localPosterColor;
      }
    } catch (e, stackTrace) {
      logErr('Failed to load series poster image for upcoming episode', e, stackTrace);
  ...
  Widget build(BuildContext context) {
    ...
    switch (Manager.settings.libColView) {
      case LibraryColorView.alwaysDominant:
        mainColor = _posterColor ?? Manager.genericGray;
        break;
      case LibraryColorView.hoverDominant:
        mainColor = _isHovering ? (_posterColor ?? Manager.genericGray) : Manager.genericGray;
        break;
    ...
    return KeyedSubtree(
      key: ValueKey('${widget.series.path}-${_posterColor?.value ?? 0}-upcoming-${widget.anilistId ?? 0}'),
                                  ...
                                  children: [
                                    Text(
                                      ...
                                      style: FluentTheme.of(context).typography.caption?.copyWith(
                                            color: _posterColor ?? FluentTheme.of(context).resources.textFillColorSecondary,
                                          ),
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                    const Spacer(),
                                    Text(
                                      ...
                                      style: FluentTheme.of(context).typography.caption?.copyWith(
                                            ...
                                            color: lighten(_posterColor ?? FluentTheme.of(context).resources.textFillColorSecondary, .4),
                                          ),
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                  ],
