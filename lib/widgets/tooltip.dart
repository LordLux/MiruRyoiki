...
Widget _defaultTooltipBuilder({
  required Widget child,
  // Add this new parameter to accept the "glass" color
...
class Tooltip extends StatefulWidget {
  ...
  final Widget Function({required Widget child})? tooltipBuilder;
...
class TooltipTheme extends InheritedTheme {
  ...
  static TooltipThemeData _getInheritedThemeData(BuildContext context) {
    ...
    return theme?.data ?? _toCustomTooltipData(FluentTheme.of(context).tooltipTheme as fluent.TooltipThemeData?);
  }

  static TooltipThemeData _toCustomTooltipData(fluent.TooltipThemeData? data) {
    if (data == null) return TooltipThemeData();
...
class TooltipThemeData with Diagnosticable {
  ...
  static TooltipThemeData lerp(
    ...
    return TooltipThemeData(
      ...
      showDuration: lerpDuration(a?.showDuration ?? Duration.zero, b?.showDuration ?? Duration.zero, t),
      textStyle: TextStyle.lerp(a?.textStyle, b?.textStyle, t),
      verticalOffset: lerpDouble(a?.verticalOffset, b?.verticalOffset, t),
      waitDuration: lerpDuration(a?.waitDuration ?? Duration.zero, b?.waitDuration ?? Duration.zero, t),
...
class _TooltipOverlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (decoration is BoxDecoration) {
      ...
      backgroundColor = (decoration as BoxDecoration).color ?? Colors.transparent;
    } else if (decoration is FrostedNoiseDecoration) {
      borderRadius = (decoration as FrostedNoiseDecoration).borderRadius;
      backgroundColor = (decoration as FrostedNoiseDecoration).backgroundColor;
    } else {
      borderRadius = BorderRadius.circular(4.0);
      backgroundColor = theme.menuColor.withOpacity(0.2);
    }
    
    final Widget textContent = Container(
      padding: padding,
    ...
    final finalBuilder = builder ?? ({required Widget child}) => _defaultTooltipBuilder(child: child, fillColor: backgroundColor, animation: animation);
        ...
        child: DefaultTextStyle.merge(
          ...
          child: Padding(
            ...
            child: ClipRRect(
              ...
              child: finalBuilder(child: textContent),
            ),
          ),
        ),
      ),
