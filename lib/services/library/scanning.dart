...
extension LibraryScanning on Library {
  ...
  Future<void> scanLocalLibraryBackground() async {
    ...
    _isLoading = true;
    scanProgress = LibraryScannerProgress(0, 1);
    notifyListeners();

    final receive = ReceivePort();

    // Create map of existing series for quick lookup
    final existingSeriesMap = {for (var s in _series) s.path: s};

    await Isolate.spawn(
      _isolateScan,
      LibraryScannerMessage(
        _libraryPath!,
        receive.sendPort,
        existingSeriesMap: existingSeriesMap,
      ),
    );
    ...
    receive.listen((msg) {
      if (msg is LibraryScannerProgress) {
        scanProgress = msg;
        print('3 | Scan progress: ${msg.processed}/${msg.total}');
        notifyListeners();
      } else if (msg is LibraryScannerResult) {
        // Merge results with existing series to preserve metadata
        _mergeScanResults(msg.series);
        _isLoading = false;
        receive.close();
        notifyListeners();
  ...
  /// Merge scan results with existing series to preserve metadata
  void _mergeScanResults(List<Series> scannedSeries) {
    // Track current series paths for comparison
    final previousSeriesPaths = _series.map((s) => s.path).toSet();

    // Identify new and removed series
    final newSeries = scannedSeries.where((s) => !previousSeriesPaths.contains(s.path)).toList();
    final scannedPaths = scannedSeries.map((s) => s.path).toSet();
    final removedSeries = _series.where((s) => !scannedPaths.contains(s.path)).toList();
  ...
  /// TODO non scannerizzare tutti quanti i file ma solo quelli che si vedrebbero nella ui (guarda regole)
  /// TODO non fare await dello scanner, o l'app rimarra' sullo splash screen per tutto il tempo dello scan
  /// TODO fare un progress bar che mostra il progresso dello scan in basso a destra
  static Future<void> _isolateScan(LibraryScannerMessage msg) async {
    ...
    // Get existing series if provided
    final existingSeriesMap = msg.existingSeriesMap ?? {};
    final results = <Series>[];

    for (final ent in allDirs) {
      try {
        final dirPath = (ent as Directory).path;
        final existingSeries = existingSeriesMap[PathString(dirPath)];

        final s = await fs.processSeries(ent, existingSeries: existingSeries);
        results.add(s);
