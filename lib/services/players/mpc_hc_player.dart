...
class MPCHCPlayer extends MediaPlayer {
  ...
  Future<void> setVolume(int level) async {
    // MPC-HC volume commands are more complex, we'll use up/down commands
    // This is a simplified implementation
    final currentVolume = await _getCurrentVolume();
    final difference = level - currentVolume;

    if (difference > 0) {
      for (int i = 0; i < difference; i++) await _sendCommand(907); // Volume up
    } else if (difference < 0) {
      for (int i = 0; i < difference.abs(); i++) await _sendCommand(908); // Volume down
    }
  }
